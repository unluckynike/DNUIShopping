...
//		          	System.out.println("----"+rs_detail.getString("content"));
	...
	public static  List<Goods> searchGodds(String inputString) throws SQLException{
		List<Goods> retList=new ArrayList<Goods>();
		Connection connection=new DBUtil().getCon();
		if (inputString!=null&&!inputString.equals("")) {
			String sqlString="SELECT * from t_goods WHERE goods_name LIKE '%" + inputString + "%'";
		
		PreparedStatement prepareStatement = connection.prepareStatement(sqlString);
		ResultSet executeQuery = prepareStatement.executeQuery();
		while(executeQuery.next()) {
			Goods goods=new Goods();
			goods.setGoodsId(executeQuery.getInt("goods_id"));
			goods.setGoodsName(executeQuery.getString("goods_name"));
			goods.setGoodsPrice(executeQuery.getFloat("goods_price"));
			goods.setGoodsPic(executeQuery.getString("goods_pic"));
			goods.setGoodsDiscount(executeQuery.getFloat("goods_discount"));  
			goods.setGoodsSales(executeQuery.getInt("goods_sales"));
			retList.add(goods);
		}
